...
export function ArcProjectCard({ project }: Props) {
  const config = SERVICE_CONFIG.arc_backend;
    ...
    <div className="glass-card rounded-xl p-6 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <a
          ...
          className={`flex items-center gap-1.5 rounded-lg border border-amber-500/20 bg-amber-500/5 px-3 py-1.5 text-xs ${config.accentClass} hover:bg-amber-500/10 transition-colors whitespace-nowrap`}
        >
          {config.ctaLabel} <ExternalLink size={10} />
        </a>
      </div>

      {/* Timeline de fases ARC */}
      ...
      {metric && Object.keys(metric.metrics as Record<string, unknown>).length > 0 && (
        <div>
          <p className="mb-2 text-xs font-semibold uppercase tracking-widest text-muted-foreground/60">
            Performance — {metric.period_label}
          </p>
